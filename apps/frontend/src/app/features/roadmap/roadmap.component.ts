...
@Component({
  ...
  template: `
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">Your Roadmap</h1>
      ...
      @if (loading()) {
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          @for (pt of programTopics(); track pt.id) {
            <a [routerLink]="['/topics', pt.topics.id]"
               class="block p-4 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 hover:border-primary-400 dark:hover:border-primary-600 hover:shadow-sm transition-all">
  ...
  async ngOnInit() {
    try {
      ...
      if (data) {
        ...
        this.programTopics.set(data.program_topics ?? []);
      }
    } catch {
      // No program yet
    } finally {
